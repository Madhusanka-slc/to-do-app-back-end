...
public class ToDoItemDAOImpl implements ToDoItemDAO {
    ...
    public ToDoItem save(ToDoItem item) throws ConstraintViolationException {
        try {
            PreparedStatement stm = connection.prepareStatement("INSERT INTO  todo_item (id,user_name,description,status) VALUES (?,?,?,?)");
            stm.setInt(1,item.getId());
            stm.setString(2,item.getUsername());
            stm.setString(3,item.getDescription());
            stm.setString(4, String.valueOf(item.getStatus()));
